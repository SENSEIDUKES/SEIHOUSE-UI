...
import { mockPlayerTracks, mockVaultFragments } from "@/components/sei/examples";
...
import { VaultFragmentCard } from "@/components/sei/particles/vault-fragment-card";
...
import { reducedMotionNotes } from "@/components/sei/styles/reduced-motion";
...
} from "./sei-native-drawer";
import { SEIMultiSelectCombobox } from "./sei-multi-select-combobox";
import { SEICommandPalette } from "./sei-command-palette";
import {
  mockDojoCategories,
  mockGenreOptions,
  mockRegistryLabels,
  ...
  return (
    <div id="behavior-hardening" className="space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        >
          <div className="space-y-2 text-sm text-[var(--sh-color-cloud)]">
            <p>Specs live in <code className="font-mono text-xs">tests/accessibility/</code>:</p>
            ...
            <div className="mt-3 flex flex-wrap gap-2 font-mono text-xs">
              ...
              <code className="rounded-md border border-white/12 bg-white/[0.05] px-2 py-1">npm run test:a11y</code>
              <code className="rounded-md border border-white/12 bg-white/[0.05] px-2 py-1">npm run test:e2e</code>
            </div>
          </div>
        </ShowcaseBlock>
      ...
      <ShowcaseBlock
        ...
        description="Stable behavior styles now live in components/sei/styles/variants.ts and are reused across components: overlay scrim, popup surfaces, interactive item states, and command group headers."
        actions={<SEIBadge variant="registry" size="sm">styles/variants.ts</SEIBadge>}
      >
